...
function DayVisitCounts({ dayPlanId }: { dayPlanId: string }) {
  const visits = useSharedPlaceVisits(dayPlanId);
...
function SharedDayCard({
  ...
  return (
    <DayCard
      dayPlan={{ dayNumber: dayPlan.day_number, date: dayPlan.date, tripId: 0, notes: "" }}
      tripId={tripId}
      placeCount={counts.total}
      visitedCount={counts.visited}
      href={`/trips/shared/${tripId}/days/${dayPlan.day_number}`}
    />
  );
}
...
export default function SharedTripDetailPage() {
  ...
  const dayPlans = useSharedDayPlans(tripId);
    ...
    <div>
      ...
      {isOwner && (
        <ConfirmDialog
          ...
          title="Delete Shared Trip"
          message={`Are you sure you want to delete "${trip.name}"? This will remove the trip for all members.`}
          confirmLabel="Delete"
          destructive
        />
      )}
